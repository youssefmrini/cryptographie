...
public class AESChiffrer {
	...
	private static byte[][] generateSubkeys(byte[] key) {
		...
		while (i < Nb * (Nr + 1)) {
			...
			for(int k = 0;k<4;k++)
				temp[k] = tmp[i-1][k]; 
			if (i % Nk == 0) { // On entre dans la condition ssi i=4,8,12.. ensuite
				
				temp = SubWord(rotateWord(temp)); //(transformation s box)
				temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] )); //On ne prend que la premiere valeur car la suite est composé de 0
				// les lignes 3, 7 et 11 .... sont les seules lignes ou on effectue le xor avec Rcon
			} 
			tmp[i] = xor_func(tmp[i - Nk], temp); // On va effectuer un XOR entre la ligne i et ligne i+3
			i++;
		}
	...
	private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) {
		...
		byte[][] tmp = new byte[state.length][state[0].length]; // Ajouter la clef au plaintext
	...
	public static byte[] encryptBloc(byte[] in) {
		...
		for (int i = 0; i < in.length; i++)
			state[i / 4][i % 4] = in[i%4*4+i/4];// rempli une matrice 
	...
	public static byte[] encrypt(byte[] in,byte[] key){
		...
		int count = 0;
		for (i = 0; i < (in.length + lenght)/16; i++){
			for (int j = 0; j < 16; j++) {
				System.out.print(bloc[j]);
			}System.out.println();
		...
		if(bloc.length == 16){
			bloc = encryptBloc(bloc);
			System.arraycopy(bloc, 0, tmp, i - 16, bloc.length); // copie un tableau à partir de l'indice (arg1) dans le tableau 2 à partir de arg2
		}
